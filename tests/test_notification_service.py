...
class TestNotificationService:
    ...
    @patch('domain.services.send_email')
    def test_send_verification_email_success(self, mock_send_email):
        ...
        # Assert
        mock_send_email.assert_called_once_with(email, token, 'verification')
    ...
    def test_send_verification_email_failure(self, mock_send_email):
        ...
        # Act & Assert
        with pytest.raises(Exception, match="Error al enviar correo de verificación: Email service error"):
            NotificationService.send_verification_email(email, token)
