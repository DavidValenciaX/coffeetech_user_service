...
def _authenticate_user(request, db):
...
def _check_user_verification_status(user, db):
...
def _send_new_verification_token(user, db):
...
def _create_user_session(user, db):
...
def _register_user_device(user, fcm_token, db):
...
def login(request, db):
    ...
    user = _authenticate_user(request, db)
    if not user:
        return create_response("error", "Credenciales incorrectas")

    # Step 2: Check user verification status
    is_verified = _check_user_verification_status(user, db)
    if not is_verified:
        _send_new_verification_token(user, db)
    ...
    try:
        session_token = _create_user_session(user, db)
        _register_user_device(user, request.fcm_token, db)
